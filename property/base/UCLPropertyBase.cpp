...
bool UCLPropertyBase::setTotalLength() {
    ...
    } else if ((totalLength + LPARTVALUE_BYTES_MIN) > UINT8_MAX &&
               (totalLength + 2 * LPARTVALUE_BYTES_MIN) <= UINT16_MAX) {
        setLPartValueBytesNum(2);
        lPart = (lPart & 0xffffffffff0000ff) | ((totalLength + 2 * LPARTVALUE_BYTES_MIN) << 8);
    } else if ((totalLength + 2 * LPARTVALUE_BYTES_MIN) > UINT16_MAX &&
               (totalLength + 3 * LPARTVALUE_BYTES_MIN) <= 0xffffff) {
        setLPartValueBytesNum(3);
        lPart = (lPart & 0xffffffff000000ff) | ((totalLength + 3 * LPARTVALUE_BYTES_MIN) << 8);
    } else if ((totalLength + 3 * LPARTVALUE_BYTES_MIN) > 0xffffff &&
               (totalLength + LPARTVALUE_BYTES_MAX) <= UINT32_MAX) {
...
void UCLPropertyBase::unpack(string property) {
    ...
    quickMatcher = (quickMatcher & 0x00ff) |
                   ((uint16_t) property[TPAER_BYTESNUM + LPARTHEAD_BYTESNUM + lPartValueBytesNum + 1] << 8);
